...
import { f as filterItemsByQuery, S as SEARCH } from "../../../chunks/params.js";
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  let result = [];
  {
    {
      result = [];
      result.push(...filterItemsByQuery(MY_PROJECTS, query).map((data) => ({
        data,
        icon: "i-carbon-cube",
        name: data.name,
        to: `projects/${data.slug}`
      })));
      result.push(...filterItemsByQuery(MY_SKILLS, query).map((data) => ({
        data,
        icon: "i-carbon-software-resource-cluster",
        name: data.name,
        to: `skills/${data.slug}`
      })));
      result.push(...filterItemsByQuery(MY_EXPERIENCES, query).map((data) => ({
        data,
